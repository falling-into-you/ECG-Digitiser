...
def get_parser():
    ...
    parser.add_argument("-d", "--database_file", type=str, required=False, default=None)
...
def parallel_transfer_files(source_paths, target_dir, move=False, num_workers=-1):
    ...
    errors = 0
    for source_path in tqdm(source_paths, desc=f"Transferring to {os.path.basename(target_dir)}", mininterval=1.0):
        try:
...
def run(args):
    ...
    if args.no_split:
        ...
        data_groups["imagesTr"] = [p for p in glob.glob(f"{args.input_data}/**/*", recursive=True) if os.path.isfile(p)]
    ...
    if args.rgba_to_rgb or args.rotate_image:
        ...
        for folder in ["imagesTr", "imagesTv", "imagesTs"]:
            ...
            files_to_consider = [f for f in possible_files if f.endswith(".png") or f.endswith(".json") or f.endswith(".hea") or f.endswith(".dat")]
